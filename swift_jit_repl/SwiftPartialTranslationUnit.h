...
struct SwiftPartialTranslationUnit {
    swift::ModuleDecl* moduleDeclaration = nullptr;  // Reference to ModuleDecl (owned by ASTContext)
    ...
    bool operator==(const SwiftPartialTranslationUnit &other) const {
        return other.moduleDeclaration == moduleDeclaration && other.TheModule == TheModule;
    }
};
